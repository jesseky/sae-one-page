<?php
	...
	if(!class_exists('SaeStorage')){ // for local or not sae env
...
<?php if($is_ok){ ?>
	...
	<?php if(!empty($attaches)){ ?>
		<div class="attaches" id="attaches">
			...
			<?php foreach($attaches as $a){ ?>
				<li onclick="toggle(event, this);"><i onclick="del(this);">×</i>
					<a href="<?php echo $a['url']; ?>" download="<?php echo $a['name']; ?>"><?php echo $a['name']; ?></a>
					<b><?php echo !empty($cache[$a['name']]) ? $cache[$a['name']] : '';?></b>
				</li>
			<?php } ?>
			</ol>
		</div>
	...
	<div class="fields">
		<form action="" method="POST" enctype="multipart/form-data" id="upload_form" onsubmit="return upload_form();">
			<p><input type="hidden" name="upload" value="upload" /><input type="file" name="file" id="file" /></p>
